 ...
 * 5 kyu - Pick peaks
 * 
 * In this kata, you will write a function that returns the positions and the values of 
...
function pickPeaks(arr) {
  ...
  for (let i = 1; i < arr.length; i++) {
    if (arr[i] > arr[i - 1]) potentialPeak = i
    else if (arr[i] < arr[i - 1] && potentialPeak !== null) {
      pos.push(potentialPeak)
      peaks.push(arr[potentialPeak])
      potentialPeak = null
    }
  }
