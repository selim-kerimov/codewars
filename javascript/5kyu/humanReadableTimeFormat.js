 ...
 * a positive integer and one of the valid units of time, separated by a space. The unit of time is 
...
function formatDuration (seconds) {
  ...
  for (const v of values) {
    const result = Math.floor(seconds / v.value)
    seconds -= result * v.value

    if (result) {
      results.push(`${result} ${v.title}${result > 1 ? 's' : ''}`)
    }
  }

  if (results.length > 1) {
    return results.slice(0, results.length - 1).join(', ') + ' and ' + results[results.length - 1]
  }
  return results[0]
}

const values = [
  { title: 'year', value: 60 * 60 * 24 * 365 },
  { title: 'day', value: 60 * 60 * 24 },
  { title: 'hour', value: 60 * 60 },
  { title: 'minute', value: 60 },
  { title: 'second', value: 1 }
]

console.log(formatDuration(32)) // 32 seconds
